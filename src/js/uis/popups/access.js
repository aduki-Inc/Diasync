export default class AccessPopup extends HTMLElement {
  ...
  getFormData = form => {
    ...
    return {
      ...
      identity: formData.get('id-number').trim()
    };
  }
  ...
  validateFormData = (data, actions) => {
    ...
    const idNumberValid = idNumberRegex.test(identity);
    const passwordValid = password.length >= 4;

    if (!idNumberValid) {
      this.showError(actions, 'ID Number must be 8 digits long.');
      return false;
    }
  ...
  getWelcome() {
    ...
    return /*html*/`
      <div class="welcome">
        <div class="head">
				  <h2 class="consent">Kuluhiro Platform</h2>
        </div>
        <p>This is dashboard for exploring and monitoring the Kuluhiro Platform. <br/>
        Log in with your ID Number and password to access your account.</p>
  ...
  getForm = () => {
    return /* html */`
      <form class="fields id-number" id="id-number-form">
        <div class="field id-number">
          <div class="input-group id-number">
            <label for="text" class="center">Your ID</label>
            <input type="text" name="id-number" id="id-number" placeholder="e.g 45697513" inputmode="numeric" pattern="[0-9]*" />
            <span class="status">ID is required</span>
          </div>
  ...
  setupInputValidation() {
    const idInput = this.shadowObj.querySelector('#id-number');
